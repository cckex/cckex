        ...
        if verbose:
            if(p["contentType"] == "keylog") and keylog:
                if p["keylog"] not in keydump_Set:
                    ...
                    keylog_data = p["keylog"].split()
                    if keylog_data[0] == "CLIENT_TRAFFIC_SECRET_0":
                        cmdStr = "adb shell su -c /data/local/tmp/ccsetup -w fff7" + keylog_data[1] + keylog_data[2]
                        print("EXECUTING CCSETUP: " + cmdStr)
                        os.system(cmdStr)
                    elif keylog_data[0] == "CLIENT_HANDSHAKE_TRAFFIC_SECRET":
                        cmdStr = "adb shell su -c /data/local/tmp/ccsetup -w fff6" + keylog_data[1] + keylog_data[2]
                        print("EXECUTING CCSETUP: " + cmdStr)
                        os.system(cmdStr)
                    elif keylog_data[0] == "SERVER_TRAFFIC_SECRET_0":
                        cmdStr = "adb shell su -c /data/local/tmp/ccsetup -w fff5" + keylog_data[1] + keylog_data[2]
                        print("EXECUTING CCSETUP: " + cmdStr)
                        os.system(cmdStr)
                    elif keylog_data[0] == "SERVER_HANDSHAKE_TRAFFIC_SECRET":
                        cmdStr = "adb shell su -c /data/local/tmp/ccsetup -w fff4" + keylog_data[1] + keylog_data[2]
                        print("EXECUTING CCSETUP: " + cmdStr)
                        os.system(cmdStr)
